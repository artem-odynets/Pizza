...
import { useState, useMemo, useEffect } from 'react'; // Додано useEffect
...
function App() {
  ...
  // 1. Ініціалізація кошика з localStorage
  const [cart, setCart] = useState(() => {
    const savedCart = localStorage.getItem('cart');
    return savedCart ? JSON.parse(savedCart) : [];
  });

  // 2. Автоматичне збереження кошика при кожній зміні
  useEffect(() => {
  ...
  return (
    <div className='container'>
      <AppContext.Provider value={{
        ...
        cart, addToCart, updateCount, removeFromCart,
